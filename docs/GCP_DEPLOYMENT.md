...
This project is set up to deploy a Next.js application on Google Cloud Run.

## Architecture

- Frontend/runtime: Next.js App Router
- Runtime process: `next start`
...
After deploy, Cloud Run will return a `run.app` URL. The container runs `next start` on port `8080`.
